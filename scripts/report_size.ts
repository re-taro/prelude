...
async function renderUsages(output: string) {
	...
	const data = Object.values(curr)
		.map((usage) => {
			...
			const diffSize = getDiff(usage.size, prevUsage?.size ?? 0);
			const diffGzipped = getDiff(usage.gzip, prevUsage?.gzip ?? 0);
			const diffBrotli = getDiff(usage.brotli, prevUsage?.brotli ?? 0);
